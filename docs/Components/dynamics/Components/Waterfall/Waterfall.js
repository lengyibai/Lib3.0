export default function ({ count = 2, gap = 10, parent = null, childs = [] } = {}) {
  if (!childs[0]) return;
  let item_width = childs[0].offsetWidth; //获取基础宽度
  let height_List = []; //存储第一行元素的高度信息
  for (let i = 0; i < childs.length; i++) {
    if (i < count) {
      parent.style.width = `${(item_width + gap) * count - gap}px`; //固定父元素宽度
      ...
      /* 更新上一行的高度，用于下一行计算 */
      height_List[minItem["minIndex"]] = parseFloat(childs[i].style.top) + parseFloat(childs[i].offsetHeight);
    }
  }
}
